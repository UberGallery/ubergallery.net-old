...
$galleryDir		= "../public/demo-gallery/ubergallery/images";			// Original images directory (No trailing slash!)
$thumbsDir		= "../public/demo-gallery/ubergallery/cache";			// Thumbnails directory (No trailing slash!)
...
$cacheDir = "../public/demo-gallery/ubergallery/cache";
...
if (file_exists($cacheFile) && time() - $cacheTime < filemtime($cacheFile) && $cacheExpire > 0) {
	...
	for ($x = $imgStart; $x < $imgEnd; $x++) {
		...
		echo "      <a href=\"/demo-gallery/ubergallery/images/$images[$x]\" title=\"$altText\" id=\"img-$x\" rel=\"colorbox\"><img src=\"/demo-gallery/ubergallery/cache/$images[$x]\" alt=\"$altText\"/></a>\r\n";
	}
	echo("    </div>\r\n");
	...
	if ($imgPerPage > 0 && $imgPerPage < $totalImages) {
		...
		echo("        <li".($currentPage > 1 ? "><a href=\"?page=$previousPage\" title=\"Previous Page\">&larr;</a>" : " class=\"inactive\">&larr;")."</li>\r\n");

		// Page links
		for ($x = 1; $x <= $totalPages; $x++) {
			echo("        <li".($x == $currentPage ? " class=\"current-page\">$x" : "><a href=\"?page=$x\" title=\"Page $x\">$x</a>")."</li>\r\n");
		}

		// Next arrow
		$nextPage = $currentPage + 1;
		echo("        <li".($currentPage < $totalPages ? "><a href=\"?page=$nextPage\" title=\"Next Page\">&rarr;</a>" : " class=\"inactive\">&rarr;")."</li>\r\n");
